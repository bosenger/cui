...
export class TaskQueueExecutionService extends EventEmitter {
  ...
  static resetInstance(): void {
    TaskQueueExecutionService.instance =
      null as unknown as TaskQueueExecutionService;
  }
  ...
  private findExecutionByStreamingId(
    streamingId: string
  ): QueueExecution | undefined {
  ...
  private broadcastTaskProgress(event: any): void {
    ...
    if (execution && execution.currentStreamingId) {
      ...
      const progressMessage = `Task queue progress: Task ${
        execution.currentTaskIndex + 1
      }/${execution.queue.tasks.length} - ${event.status}`;
  ...
  async executeQueue(queueId: string): Promise<void> {
    ...
    if (this.runningExecutions.has(queueId)) {
      throw new CUIError(
        ...
        "Queue is already being executed",
        400
      );
    }
  ...
  private async executeNextTask(execution: QueueExecution): Promise<void> {
    ...
    if (execution.isExecutingTask) {
      this.logger.debug(
        "Task already executing for queue, skipping duplicate call",
        {
          queueId,
          currentTaskIndex,
        }
    ...
    try {
      ...
      const config = await this.prepareConversationConfig(
        currentTask,
        execution
      );

      // 使用一个Promise来捕获session-started事件
      let streamingId: string;
      let capturedSessionId: string | undefined;

      // 设置一个通用的监听器来捕获任何session-started事件
      const sessionStartedListener = ({
        streamingId: eventStreamingId,
        claudeSessionId,
      }: {
        streamingId: string;
        claudeSessionId: string;
      }) => {
        console.log("📡 收到session-started事件:", {
          eventStreamingId,
          claudeSessionId,
        });
        // 暂时存储，等待验证是否是我们的streamingId
        if (!capturedSessionId) {
          capturedSessionId = claudeSessionId;
          console.log("💾 暂存sessionId:", claudeSessionId);
        }
      };

      // 在启动对话前设置监听器
      console.log("🎯 设置session-started监听器（启动前）");
      this.statusManager.once("session-started", sessionStartedListener);

      try {
        // Start the Claude conversation
        console.log("🚀 开始启动Claude对话...");
        const result = await this.claudeProcessManager.startConversation(
          config
        );
        streamingId = result.streamingId;
        console.log("📋 Claude对话已启动，streamingId:", streamingId);

        // 检查是否已经捕获到sessionId
        if (capturedSessionId) {
          console.log("✅ 已捕获sessionId:", capturedSessionId);
          execution.currentSessionId = capturedSessionId;
          ...
          console.log("🔄 任务状态已更新为运行中，包含sessionId");
        } else {
          console.log("⚠️ 未立即捕获到sessionId，将继续等待");
        }
      } catch (error) {
        console.error("❌ 启动Claude对话失败:", error);
        throw error;
      }
      ...
      // 如果还没有sessionId，则只用streamingId更新任务状态
      if (!execution.currentSessionId) {
        console.log("🔄 用streamingId更新任务状态为运行中");
        await this.taskQueueService.updateTaskStatus(
          currentTask.id,
          "running",
        ...
        setTimeout(() => {
          const immediateSessionId =
            this.statusManager.getSessionId(streamingId);
          if (immediateSessionId && !execution.currentSessionId) {
            console.log("🔄 后备检查获取到sessionId:", immediateSessionId);
            execution.currentSessionId = immediateSessionId;
            ...
            this.taskQueueService
              ...
              .catch((error) => {
                console.error("❌ 后备更新任务sessionId失败:", error);
              });
          }
        }, 2000); // 等待2秒后检查
      }
      ...
      await this.handleTaskError(
        execution.currentStreamingId || "",
        error instanceof Error ? error.message : String(error)
      );
    }
  }

  private async prepareConversationConfig(
    task: Task,
    ...
    if (task.type === "fork" && execution.currentSessionId) {
      ...
      try {
        ...
        const previousSessionMessages =
          await this.claudeProcessManager.getConversationHistory(
        ...
        this.logger.warn(
          "Failed to get previous session history for fork, proceeding as new session",
          {
            taskId: task.id,
            previousSessionId: execution.currentSessionId,
  ...
  private async handleTaskCompletion(
    ...
    if (!execution) {
      this.logger.debug("Received task completion for non-queue session", {
        streamingId,
      });
      return;
    }
    ...
    console.log("🔍 [TaskExecution] 任务完成处理:");
    console.log("  streamingId:", streamingId);
    console.log(
      "  当前execution.currentSessionId:",
      execution.currentSessionId
    );
    ...
    if (success) {
      // 使用已经保存的sessionId
      const sessionIdToUse = execution.currentSessionId;
      console.log("🔄 更新任务状态为完成，使用sessionId:", sessionIdToUse);
  ...
  private async handleTaskError(
    ...
    const execution = this.findExecutionByStreamingId(streamingId);
    if (!execution) {
      this.logger.debug("Received task error for non-queue session", {
        streamingId,
      });
      return;
    }
    ...
    await this.handleQueueError(
      execution,
      `Task ${execution.currentTaskIndex + 1} failed: ${errorMessage}`
    );
  }
  ...
  private async handleQueueError(
    execution: QueueExecution,
    errorMessage: string
  ): Promise<void> {
    const completedAt = new Date().toISOString();
    ...
    for (
      let i = execution.currentTaskIndex + 1;
      i < execution.queue.tasks.length;
      i++
    ) {
  ...
  async cancelQueue(queueId: string): Promise<void> {
    ...
    if (!execution) {
      throw new CUIError(
        "QUEUE_NOT_RUNNING",
        "Queue is not currently running",
        400
      );
    }
    ...
    if (execution.currentStreamingId) {
      try {
        await this.claudeProcessManager.stopConversation(
          execution.currentStreamingId
        );
      } catch (error) {
        this.logger.warn(
          "Failed to stop current task during cancellation",
          error
        );
      }
    }
    ...
    await this.handleQueueError(
      execution,
      "Queue execution was cancelled by user"
    );
  }
  ...
  getQueueProgress(
    ...
    return {
      ...
      percentage: Math.round(
        (execution.currentTaskIndex / execution.queue.tasks.length) * 100
      ),
    };
  }
}
