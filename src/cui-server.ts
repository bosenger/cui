...
export class CUIServer {
  ...
  constructor(configOverrides?: {
    ...
    this.workingDirectoriesService = new WorkingDirectoriesService(
      this.historyReader,
      this.logger
    );
    ...
    this.permissionTracker.setConversationStatusManager(
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      this.logger.debug(
        "Task queue execution service initialized successfully"
      );
      ...
      try {
        const mcpConfigPath = await this.mcpConfigGenerator.generateConfig(
          this.port
        );
        this.processManager.setMCPConfigPath(mcpConfigPath);
        this.logger.debug("MCP config generated and set", {
          path: mcpConfigPath,
        });
      } catch (error) {
        ...
        if (isTestEnv) {
          ...
          this.logger.error(
            "MCP config generation failed in production environment",
            {
              error: error instanceof Error ? error.message : String(error),
            }
          );
          throw new CUIError(
            "MCP_CONFIG_REQUIRED",
            `MCP server files are required in production but not found: ${
      ...
      this.configService.onChange(async (newConfig) => {
        try {
          ...
          this.logger.error(
            "Failed to reload router after config change",
            error
          );
        }
      });
        ...
        if (isDev && ViteExpress) {
          try {
            this.server = this.app.listen(this.port, this.host, () => {
              this.logger.debug(
                "Server successfully bound to port (dev mode)",
                {
                  port: this.port,
                  host: this.host,
            ...
            this.logger.info(
              `CUI development server running on http://${this.host}:${this.port}`
            );
          } catch (error) {
  ...
  async stop(): Promise<void> {
    ...
    if (activeSessions.length > 0) {
      ...
      this.logger.debug("Active sessions to stop", {
      ...
      const stopResults = await Promise.allSettled(
        activeSessions.map((streamingId) =>
          this.processManager
            ...
            .catch((error) =>
              this.logger.error(`Error stopping session ${streamingId}:`, error)
            )
        )
      );
  ...
  private setupRoutes(): void {
    ...
    this.app.use(
      "/api/system",
      createSystemRoutes(this.processManager, this.historyReader)
    );
    this.app.use(
      "/",
      createSystemRoutes(this.processManager, this.historyReader)
    ...
    this.app.use(
      "/api/permissions",
      createPermissionRoutes(this.permissionTracker)
    );
    // Notifications routes - before auth (needed for service worker subscription on first load)
    this.app.use(
      "/api/notifications",
      createNotificationsRoutes(this.webPushService)
    );
    ...
    if (!this.configOverrides?.skipAuthToken) {
      ...
      this.logger.warn(
        "Authentication middleware is disabled - API endpoints are not protected!"
      );
    }
    ...
    this.app.use(
      "/api/filesystem",
      createFileSystemRoutes(this.fileSystemService)
    );
    ...
    this.app.use(
      "/api/task-queues",
      createTaskQueueRoutes(
        this.taskQueueService,
        this.taskQueueExecutionService
      )
    );
  ...
  private setupProcessManagerIntegration(): void {
    this.logger.debug(
      "Setting up ProcessManager integration with StreamManager"
    );
    ...
    this.processManager.on("claude-message", ({ streamingId, message }) => {
      ...
      if (message && message.type === "system" && message.subtype === "init") {
        this.logger.debug(
          "Skipping broadcast of system init message (included in API response)",
          {
            streamingId,
    ...
    this.processManager.on("process-closed", ({ streamingId, code }) => {
      this.logger.debug(
        "Received process-closed event, closing StreamManager session",
        {
          streamingId,
          exitCode: code,
      ...
      this.logger.debug("Unregistering session from status tracker", {
        streamingId,
      });
      ...
      const removedCount =
        this.permissionTracker.removePermissionsByStreamingId(streamingId);
      if (removedCount > 0) {
    ...
    this.processManager.on("process-error", ({ streamingId, error }) => {
      this.logger.debug(
        "Received process-error event, forwarding to StreamManager",
        {
          streamingId,
          error,
      ...
      this.logger.debug(
        "Unregistering session from status tracker due to error",
        { streamingId }
      );
      this.statusTracker.unregisterActiveSession(streamingId);
  ...
  private async initializeOrReloadRouter(
    ...
    if (!config.router?.enabled) {
      if (this.routerService) {
        this.logger.info(
          "Router disabled in configuration, stopping router service"
        );
        await this.routerService.stop();
    ...
    try {
      ...
      if (this.routerService) {
        this.logger.info(
          "Reloading router service due to configuration change..."
        );
        await this.routerService.stop();
        this.routerService = undefined;
      } else {
        this.logger.debug(
          "Router service is enabled, attempting to initialize..."
        );
      }
      ...
      this.logger.error(
        "Router initialization failed, continuing without router",
        error
      );
    }
  }
